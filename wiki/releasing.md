...
> History: **v0.2.1 (2026-09-05) was published by the workflow via OIDC trusted publishing with a signed provenance statement** (`npm notice publish Signed provenance statement … transparency log`), so path 1 is configured and working; keep `NPM_TOKEN` unset. Earlier, the `NPM_TOKEN` set on 2026-04-28 never published — both the v0.1.0 and v0.2.0 release runs failed at `npm publish` with `E404 … PUT https://registry.npmjs.org/swiggy-cli`, which is how the registry reports an unauthorized/expired token. 0.1.0–0.1.4 were published locally.
